...
7.   В тестовом задании предлагается отправлять userId в теле запроса на эндпоинт GET /api/internal/bet для тестирования получения рекомендуемой ставки от API. Однако, согласно стандартам HTTP, метод GET не поддерживает тело запроса, и большинство серверов и прокси его игнорируют.
Поэтому я изменил запрос и направляю его без тела. Несмотря на это, API возвращает 200 OK.
Наиболее правильным решением было бы использовать POST, поскольку он допускает тело запроса, но в тестовом задании уже присутствует эндпоинт POST /api/internal/auth. 
